...
def help_text(mode: str | None) -> str:
    platforms = "Twitch, YouTube, Kick, VK Video, Rutube, Instagram и TikTok"
    common = (
        f"📺 Поддерживаемые площадки\n{platforms}.\n\n"
        "🔎 Проверить эфиры — запускает проверку сразу.\n"
        "📋 Мои подписки — показывает добавленные каналы; там их можно "
        "посмотреть или удалить.\n"
        "🔄 Режим — переключает между режимами.\n\n"
        "Первый опрос после добавления только запоминает текущий статус. "
        "Уведомление придёт при следующем переходе канала из офлайна в эфир."
    )
    if mode == "subscriber":
        return (
            "🔔 Режим: Подписчик\n\n"
            "Нажми «Добавить канал», выбери площадку и отправь ссылку на "
            "стримера. Подписка создаётся для этого личного чата — выбирать "
            "группу или канал не нужно.\n\n"
            "Когда стример начнёт эфир, бот пришлёт личное уведомление с "
            "именем стримера, площадкой, названием эфира и ссылкой.\n\n"
            + common
            + "\n\nКоманды: /add, /list, /remove, /check, /subscriptions, /start."
        )
    return (
        "🎙 Режим: Стример\n\n"
        "1. Добавь бота администратором в нужный Telegram-канал или группу. "
        "Для форума открой нужную тему и отправь в ней /topic.\n"
        "2. В личном чате нажми «Добавить канал», выбери площадку, отправь "
        "ссылку и выбери получателя уведомлений.\n"
        "3. В «Оформлении» можно изменить заголовок и описание, выбрать "
        "превью и блюр, добавить ссылки-кнопки, настроить их названия, "
        "эмодзи, строки и цвета.\n\n"
        "Уведомления в группе или канале обновляются, если несколько эфиров "
...
async def menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if data == "menu:help":
        await render_ui(
            update,
            context,
            help_text(database.get_user_mode(update.effective_user.id)),
            InlineKeyboardMarkup(
                [[InlineKeyboardButton("В меню", callback_data="menu:home")]]
            ),
        )
